...
namespace yazYapım
{
    public partial class form_story : Form
    {
        ...
        SqlConnection baglanti = new SqlConnection("Data Source=DESKTOP-83U1NBD;Initial Catalog=VeriTabani_Scrum;Integrated Security=True");
        ...
        private void form_story_Load(object sender, EventArgs e)
        {
            //Story Oluşumunu panele ekliyor.
             if (baglanti.State == ConnectionState.Closed)
             {
                 ...
                 foreach (DataRow drstory in dtable.Rows)
               {
                   story_i++;
                     ...
                     btn_story.Text = " # " + drstory["adi"].ToString() + "\n" + drstory["aciklamasi"].ToString();
                     btn_story.AllowDrop=true;
                     btn_story.BackColor = Color.FromArgb(Convert.ToInt32(drstory["renk_A"]), Convert.ToInt32(drstory["renk_R"]), Convert.ToInt32(drstory["renk_G"]), Convert.ToInt32(drstory["renk_B"]));
            ...
            //Task oluşumu panele ekleme
             if (baglanti.State == ConnectionState.Closed)
             {
                 ...
                 foreach (DataRow drtask in dtable_task.Rows)
                 {
                     task_i++;
                     ...
                     btn_task.Text = "# " + drtask["adi"].ToString() + "\n" + drtask["sorumlusu"].ToString();
                     btn_task.AllowDrop = true;
                     btn_task.BackColor = Color.FromArgb(Convert.ToInt32(drtask["renk_A"]), Convert.ToInt32(drtask["renk_R"]), Convert.ToInt32(drtask["renk_G"]), Convert.ToInt32(drtask["renk_B"]));
